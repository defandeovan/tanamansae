...
$stmt = $koneksi->prepare("
    SELECT u.username, u.email, ud.nama, ud.phone, ud.gender, ud.profil, ud.provinsi, ud.kota, ud.jalan, ud.kode_pos, s.store_name 
    FROM users u 
...
if (isset($_POST['simpan'])) {
    ...
    $profil = $row['profil']; // Set default profil

    if (isset($_FILES['pprofil']) && $_FILES['pprofil']['error'] == 0) {
    ...
    // Update userdetails
    $stmt = $koneksi->prepare("UPDATE userdetails SET nama = ?, gender = ?, profil = ?, provinsi = ?, kota = ?, jalan = ?, kode_pos = ? WHERE user_id = (SELECT user_id FROM users WHERE username = ?)");
    $stmt->bind_param("ssssssss", $nama, $gender, $profil, $provinsi, $kota, $jalan, $kode_pos, $username);
        ...
        <main>
            <div class="profile-form">
                ...
                <div class="input">
                    ...
                    <div>
                        <input type="text" id="store-name" name="store-name" value="<?php echo isset($row['store_name']) ? htmlspecialchars($row['store_name']) : ''; ?>">
                    </div>
                </div>
                <div>
                ...
                <div class="gender-input">
                    <label><input type="radio" name="gender" value="male" <?php echo isset($row['gender']) && $row['gender'] == 'male' ? 'checked' : ''; ?>> Laki-laki</label>
                    <label><input type="radio" name="gender" value="female" <?php echo isset($row['gender']) && $row['gender'] == 'female' ? 'checked' : ''; ?>> Perempuan</label>
                    <label><input type="radio" name="gender" value="other" <?php echo isset($row['gender']) && $row['gender'] == 'other' ? 'checked' : ''; ?>> Lainnya</label>
                </div>
                <div class="input">
                    <div>
